...
func ReadBorReceipt(db ethdb.Reader, hash common.Hash, number uint64, config *params.ChainConfig) *types.Receipt {
	...
	log.Info("[debug] [db] config check done")

	// We're deriving many fields from the block body, retrieve beside the receipt
	borReceipt := ReadRawBorReceipt(db, hash, number)
	log.Info("[debug] [db] bor receipt fetched", "borReceipt", borReceipt == nil)
	// if borReceipt == nil {
	// 	return nil
	// }
	...
	// 	return nil
	// }
	log.Info("[debug] [db] receipts fetched", "receipts", receipts == nil)
	...
	log.Info("[debug] [db] body fetched")
